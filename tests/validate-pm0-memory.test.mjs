...
test("validatePm0Memory warns about malformed artifacts and placeholder residue", async () => {
  ...
  try {
    ...
    await write(root, ".pm0/prds/2026-05-23-onboarding-empty-state.md", "# PRD\n\n## Product Intent\n");
    ...
    assert.match(messages, /Proposal 2026-05-23-onboarding-empty-state\.md is missing required section: ## Handoff Readiness/);
    assert.match(messages, /PRD 2026-05-23-onboarding-empty-state\.md is missing required section: ## Requirements/);
    assert.match(messages, /PRD 2026-05-23-onboarding-empty-state\.md is not linked from any surface Accepted PRDs section/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
