...
test('context reference supports focused tool-backed research', async () => {
  ...
  assert.match(
    context,
    /competitor analysis, market research, alternatives, category scans, pricing benchmarks/i,
  )
...
test('handoff reference has exactly three outcomes', async () => {
  ...
  assert.deepEqual(outcomes, ['accepted for engineering', 'rejected', 'needs more discussion'])
})
...
test('skill guidance does not hard-code source-tree script paths', async () => {
  const files = ['skills/pm0/reference/init.md', 'skills/pm0/reference/github-ci.md']
  ...
  for (const file of files) {
    ...
    assert.match(
      text,
      /active PM0 skill directory|PM0 skill directory committed in the repository|installed PM0 skill directory/,
    )
  }
})
